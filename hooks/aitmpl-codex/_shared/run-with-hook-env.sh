...
if [[ "$CODEX_HOOK_EVENT_NAME" == "PreToolUse" ]]; then
  ...
  if [[ -n "$tool_input_raw" && "$tool_input_raw" != "null" ]]; then
    ...
    if [[ -n "$tool_input_json" ]]; then
      ...
      export CODEX_TOOL_FILE_PATH="$(jq -r 'try (fromjson | .file_path // .path // .destination // .params.file_path // .params.path // empty) catch ""' <<<"$tool_input_raw" 2>/dev/null || true)"
      export CODEX_TOOL_COMMAND="$(jq -r 'try (fromjson | .command // (.params.command | join(" ")) // empty) catch ""' <<<"$tool_input_raw" 2>/dev/null || true)"
    fi
  fi
fi
